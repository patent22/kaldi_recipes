...
if $position_dependent_phones; then
  ...
  cat <(set -f; for x in `cat $srcdir/silence.txt`; do for y in "" "" "_B" "_E" "_I" "_S"; do echo -n "$x$y "; done; echo; done) \
    <(set -f; for x in `cat $srcdir/one_state.txt`; do for y in "" "_B" "_E" "_I" "_S"; do echo -n "$x$y "; done; echo; done) \
    <(set -f; for x in `cat $srcdir/three_state.txt`; do for y in "" "_B" "_E" "_I" "_S"; do echo -n "$x$y "; done; echo; done) \
    <(set -f; for x in `cat $srcdir/five_state.txt`; do for y in "" "_B" "_E" "_I" "_S"; do echo -n "$x$y "; done; echo; done) \
  ...
  cat $srcdir/silence.txt $srcdir/one_state.txt $srcdir/three_state.txt $srcdir/five_state.txt | \
...
if $share_silence_phones; then
  ...
  cat $srcdir/silence.txt | awk '{printf("%s ", $0); } END{printf("\n");}' | cat - $srcdir/{one,three,five}_state.txt | \
  ...
  cat $srcdir/silence.txt $srcdir/one_state.txt $srcdir/three_state.txt $srcdir/five_state.txt | utils/apply_map.pl $tmpdir/phone_map.txt > $dir/phones/sets.txt
  cat $dir/phones/sets.txt | awk '{print "shared", "split", $0;}' > $dir/phones/roots.txt
fi

cat $srcdir/silence.txt | utils/apply_map.pl $tmpdir/phone_map.txt | \
  awk '{for(n=1;n<=NF;n++) print $n;}' > $dir/phones/silence.txt
cat $srcdir/one_state.txt | utils/apply_map.pl $tmpdir/phone_map.txt | \
  awk '{for(n=1;n<=NF;n++) print $n;}' > $dir/phones/one_state.txt
  cat $srcdir/three_state.txt | utils/apply_map.pl $tmpdir/phone_map.txt | \
awk '{for(n=1;n<=NF;n++) print $n;}' > $dir/phones/three_state.txt
  cat $srcdir/five_state.txt | utils/apply_map.pl $tmpdir/phone_map.txt | \
awk '{for(n=1;n<=NF;n++) print $n;}' > $dir/phones/five_state.txt
...
cat $srcdir/extra_questions.txt > $dir/phones/extra_questions.txt
...
if $position_dependent_phones; then
  for suffix in _B _E _I _S; do
    (set -f; for x in `cat $srcdir/silence.txt`; do echo -n "$x$suffix "; done; echo) >>$dir/phones/extra_questions.txt
  done
  for suffix in "" _B _E _I _S; do
    (set -f; for x in `cat $srcdir/one_state.txt`; do echo -n "$x$suffix "; done; echo) >>$dir/phones/extra_questions.txt
  done
  for suffix in "" _B _E _I _S; do
    (set -f; for x in `cat $srcdir/three_state.txt`; do echo -n "$x$suffix "; done; echo) >>$dir/phones/extra_questions.txt
  done
  for suffix in "" _B _E _I _S; do
    (set -f; for x in `cat $srcdir/five_state.txt`; do echo -n "$x$suffix "; done; echo) >>$dir/phones/extra_questions.txt
  done
fi
...
if [[ ! -z $phone_symbol_table ]]; then
  ...
  echo "<eps>" | cat - $dir/phones/{silence,one_state,three_state,five_state}.txt | awk -v f=$phone_symbol_table '
  ...
  echo "<eps>" | cat - $dir/phones/{silence,one_state,three_state,five_state,disambig}.txt | \
...
if $position_dependent_phones; then
  cat $dir/phones/{silence,one_state,three_state,five_state}.txt | \
...
for f in silence one_state three_state five_state optional_silence disambig context_indep; do
...
cat $dir/phones/extra_questions.txt | sed -e '6d;11d;16d' > $dir/phones/tmp.txt; cat $dir/phones/tmp.txt > $dir/phones/extra_questions.txt
rm $dir/phones/tmp.txt
...
onelist=`cat $dir/phones/one_state.csl`
threelist=`cat $dir/phones/three_state.csl`
fivelist=`cat $dir/phones/five_state.csl`
utils/gen_topo_4phonesets.pl $num_sil_states $num_one_states $num_three_states $num_five_states $silphonelist $onelist $threelist $fivelist > $dir/topo
cat $dir/topo | sed -e '9d;19d' > $dir/tmp.txt; cat $dir/tmp.txt > $dir/topo
rm $dir/tmp.txt
