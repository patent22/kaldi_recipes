...
context_opts="--pdf-class-list=0"  # e.g. set this to "--context-width 5 --central-position 2" for quinphone.
...
if [ $stage -le -4 ] && $train_tree; then
  ...
  $cmd JOB=1:$nj $dir/log/acc_tree.JOB.log \
    acc-tree-stats $tree_stats_opts $phone_map_opt --ci-phones=$ciphonelist $alidir/final.mdl "$feats" \
...
if [ $stage -le -3 ] && $train_tree; then
  ...
  compile-questions $compile_questions_opts $lang/topo $dir/questions.int $dir/questions.qst 2>$dir/log/compile_questions.log || exit 1;
