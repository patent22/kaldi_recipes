...
mfccdir=mfcc
for set in train test; do
	 steps/make_mfcc.sh $curdir/data/$set $curdir/exp/make_mfcc/$set $mfccdir
# Compute cmvn. (This steps should be processed right after mfcc features are extracted.)
	 ...
	 steps/compute_cmvn_stats.sh $curdir/data/$set $curdir/exp/make_mfcc/$set $curdir/$mfccdir
done
...
# for set in train test; do
#	 	steps/make_plp.sh $curdir/data/$set $curdir/exp/make_plp/$set $plpdir
...
# 	steps/compute_cmvn_stats.sh $curdir/data/$set $curdir/exp/make_plp/$set $curdir/$plpdir
# done
...
steps/align_si.sh $mono_align_opt $curdir/data/train $curdir/data/lang $curdir/exp/mono $curdir/exp/mono_ali || exit 1
...
echo "SGMM2 aligning options: $sggm2_align_opt"
...
steps/decode_sgmm2.sh $sgmm2_train_decode_opt $curdir/exp/sgmm/graph $curdir/data/train $curdir/exp/sgmm/decode_train
steps/decode_sgmm2.sh $sgmm2_test_decode_opt $curdir/exp/sgmm/graph $curdir/data/test $curdir/exp/sgmm/decode_test
...
echo "SGMM2+MMI train-decoding options: $sgmmi_train_decode_opt"
echo "SGMM2+MMI test-decoding options: $sgmmi_test_decode_opt"
...
steps/decode_sgmm2_rescore.sh $sgmmi_train_decode_opt $curdir/data/lang $curdir/data/train $curdir/exp/sgmm/decode_train $curdir/exp/sgmm_mmi/decode_train
steps/decode_sgmm2_rescore.sh $sgmmi_test_decode_opt $curdir/data/lang $curdir/data/test $curdir/exp/sgmm/decode_test $curdir/exp/sgmm_mmi/decode_test
...
steps/nnet2/decode.sh $dnn1_train_decode_opt $curdir/exp/tri3/graph $curdir/data/train $curdir/exp/tri4/decode_train
steps/nnet2/decode.sh $dnn1_test_decode_opt $curdir/exp/tri3/graph $curdir/data/test $curdir/exp/tri4/decode_test
